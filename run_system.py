...
def main(config_path: str = "config.json"):
    ...
    print("=" * 80)
    print("FuSNet-output Kalman ReTM System")
    print("=" * 80)
    ...
    print("Kalman model:")
    print("  u(t)      = FuSNet(mB)(t)")
    print("  mA_hat(t) = R(t) u(t)")
    ...
    if len(qa_mics) != len(default_qa_mics) or len(qb_mics) != len(default_qb_mics):
        raise ValueError(
            f"For mic_config={mic_config}, expected "
            f"{len(default_qa_mics)} QA mics and {len(default_qb_mics)} QB mics, "
    ...
    print("\n[4] Running Kalman ReTM using FuSNet output as input...")

    mode = kal_cfg.get("mode", "block").lower()

    common_kwargs = dict(
        qb=mB.shape[0],                 # kept for compatibility
        qa=mA.shape[0],
        L=int(kal_cfg.get("L", 256)),
    ...
    mA_hat, kalman_difference, err = kf.process(
        mB=mB,
        mA=mA,
        mA_fusnet=mA_f,
    )

    print_basic_metrics(mA, mA_hat, name="FuSNet-output Kalman final estimate")
    ...
    if bool(out_cfg.get("save_npy", True)):
        ...
        np.save(out_dir / "kalman_difference_from_fusnet.npy", kalman_difference)
    ...
    if bool(out_cfg.get("save_error_plot", True)):
        ...
        plt.title("FuSNet-output Kalman estimation error over time")
    ...
    if bool(out_cfg.get("save_wav", True)):
        ...
        write_mic_wavs(
            out_dir / "wav_kalman_difference",
            kalman_difference,
            fs,
            prefix="kalman_difference_mic",
        )
    ...
    metrics = {
        ...
        "num_samples": int(T),
        "mode": mode,
        "kalman_input": "FuSNet Group-A output",
        "kalman_filter_dimension": f"{mA.shape[0]} x {mA.shape[0]} x {int(kal_cfg.get('L', 256))}",
        "L": int(kal_cfg.get("L", 256)),
        "block_size": int(kal_cfg.get("block_size", 128)),
        "transition": float(kal_cfg.get("transition", 0.995)),
        "process_noise": float(kal_cfg.get("process_noise", 1e-7)),
