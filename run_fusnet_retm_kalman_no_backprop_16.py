...
from retm_kalman.fusnet_inference_16 import (
    load_fusnet16_model,
...
def main():
    seq_dir = Path(
        "/home/jaliya/eeg_speech/Julian/RetM_Workspace/Dataset/Dataset_Folder2/Moving/L/1"
    )
    checkpoint_path = Path(
        "/home/jaliya/eeg_speech/Julian/RetM_Workspace/ReTM_Research_Project/"
        "best_checkpoint_A1_1_FUSENet_16_L.pth"
    )
    out_dir = Path("results_fusnet_retm_kalman_L_1_16")
    out_dir.mkdir(parents=True, exist_ok=True)

    fs_target = 16000
    qa_mics = [1, 2, 3, 4, 5, 6, 7]
    qb_mics = [8, 9, 10, 11, 12, 13, 14, 15, 16]
    context = 4096
    ...
    print("=" * 80)
    print("FuSNet ReTM Kalman — 16-mic moving-source adaptation")
    print("=" * 80)
    ...
    model, _ = load_fusnet16_model(
        checkpoint_path=checkpoint_path, context=context, device=str(device),
    )
    model.eval()

    mA_fusnet = predict_fusnet16_original_style(
