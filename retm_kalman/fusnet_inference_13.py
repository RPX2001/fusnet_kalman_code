...
from .fusnet_model_13 import build_fusnet13_from_context, MultiChannelConvolutionModel13
...
@torch.no_grad()
def predict_fusnet13_original_style(
    model: MultiChannelConvolutionModel13,
    mB: np.ndarray,
    context: int = 4096,
    ...
    """FuSNet inference for the 13-mic setup."""
    dev = torch.device(device if torch.cuda.is_available() and str(device).startswith("cuda") else "cpu")
    mB = np.asarray(mB, dtype=np.float32)
    if mB.ndim != 2 or mB.shape[0] != 8:
        raise ValueError(f"Expected mB shape [8, T], got {mB.shape}")
