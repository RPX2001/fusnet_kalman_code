...
from .fusnet_model_16 import build_fusnet16_from_context, MultiChannelConvolutionModel16
...
@torch.no_grad()
def predict_fusnet16_original_style(
    model: MultiChannelConvolutionModel16,
    mB: np.ndarray,
    context: int = 4096,
    ...
    """FuSNet inference for the 16-mic setup."""
    dev = torch.device(device if torch.cuda.is_available() and str(device).startswith("cuda") else "cpu")
    mB = np.asarray(mB, dtype=np.float32)
    if mB.ndim != 2 or mB.shape[0] != 9:
        raise ValueError(f"Expected mB shape [9, T], got {mB.shape}")
